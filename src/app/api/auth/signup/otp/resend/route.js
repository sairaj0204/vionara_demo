...
import { SignupOtpError, resendSignupOtpCode } from '@/lib/signup-otp';
...
export async function POST(req) {
    if (!hasMongoConfig()) {
        return NextResponse.json(
            { success: false, message: 'MONGODB_URI is required to resend OTP.' },
            { status: 503 }
        );
    }
    ...
    try {
        ...
        const result = await resendSignupOtpCode(payload);
